...
assert_links() {
  ...
  while IFS=$'\t' read -r package relative_path; do
    ...
    case "$expected_kind" in
      runtime)
        [[ "$resolved" == "$TEST_HOME/terminalenv/dotpkgs/$package/$relative_path" ]] || fail "$path resolved to $resolved, expected $TEST_HOME/terminalenv/dotpkgs/$package/$relative_path"
        ;;
      store)
...
run_home_manager_mode() {
  ...
  [[ -e "$generation/home-files/.config/fish/hm-generated.fish" ]] || fail "Missing generated fish config"
}
